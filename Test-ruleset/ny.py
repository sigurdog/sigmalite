ubuntu@scada-slave:~/IEC-60870-5-104/IEC-104-Python/Python$ cat iec104servertest.py
'''
/******************************************************************************
*
...
def main():
    ...
    while(True):
        ...
        sIEC104Config.sServerSet.sServerConParameters.ai8SourceIPAddress = "10.20.0.133".encode('utf-8') # own server bind address
...
#        while True:
 #           cmd = input("Enter command (s = send command, x = exit): ").strip().lower()
  #          if cmd == 'x':
   #             print("x pressed, exiting loop")
    #            break
     #       elif cmd == 's':
      #          print("s pressed, send command called")
       #         sendCommand(myClient)
        #    time.sleep(0.1)
        #break

        while True:
            cmd = input("Enter command (u = update, x = exit): ").strip().lower()
            if cmd == 'x':
                print("x pressed, exiting main program")
                break
            elif cmd == 'u':
